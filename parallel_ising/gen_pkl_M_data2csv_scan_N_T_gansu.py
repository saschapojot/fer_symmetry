...
#this script creates slurm bash files for running data2csv/pkl_M_data2csv.py separately
...
outPath="./bashFiles_pkl_M_data2csv/"
...
def contents_to_bash(bN, T, file_index):
    ...
    contents=[
        ...
        "#SBATCH -p lzicnormal\n",
        "#SBATCH --mem=6GB\n",
        f"#SBATCH -o out_pkl_M_data2csv_N{NStr}_{TStr}.out\n",
        f"#SBATCH -e out_pkl_M_data2csv_N{NStr}_{TStr}.err\n",
        "cd  /public/home/hkust_jwliu_1/liuxi/Documents/cppCode/fer_symmetry/parallel_ising\n",
        f"python3 -u ./data2csv/pkl_M_data2csv.py {NStr} {TStr} {init_path}  {which_row} {startingFileIndSuggest} {sweep_to_write} {lag} {sweep_multiple}\n"

    ]
    out_chunk=outPath+f"/chunk{file_index // chunk_size}/"
    Path(out_chunk).mkdir(exist_ok=True,parents=True)
    outBashName=out_chunk+f"/pkl_M_data2csv_N{NStr}_T{TStr}.sh"
