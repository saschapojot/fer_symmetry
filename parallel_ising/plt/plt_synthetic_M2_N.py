...
NVec=[10,14,20,24,30,60,120]
TVec=np.linspace(0.6,1.12,40)
def T_M_to_rescaled(TToPlt_one_N,M2_to_plot_one_N,N,beta,nu):
    ...
    M_rescaled=np.sqrt(M2_to_plot_one_N)*N**(beta/nu)
    lmd=np.abs(TToPlt_one_N-Tc)/Tc*N**(1/nu)
    return lmd, M_rescaled
...
for ind,N in enumerate(NVec):
    ...
    lmd, M_rescaled=T_M_to_rescaled(TToPlt_one_N,M2_to_plot_one_N,N,beta,nu)
    plt.scatter(lmd,M_rescaled,label=f"N={N}",s=4)
plt.xscale("log")
plt.yscale("log")
plt.xlabel(r"$\lambda$")
plt.ylabel(r"scaled M")
plt.legend(loc="best")
