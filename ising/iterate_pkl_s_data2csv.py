...
from decimal import Decimal, getcontext
#this script runs data2csv/pkl_s_data2csv.py sequentially
...
#T_vec1 is one group, T_vec2 is one group, T_vec3 is one group
...
for TStr in TStrAll:
    try:
        ...
        current_process = subprocess.Popen(
            ["python3", "./data2csv/pkl_s_data2csv.py", f"{N}", f"{TStr}", f"{init_path}", str(row)]
        )
